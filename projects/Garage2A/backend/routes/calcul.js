...
router.post('/', async (req, res) => {
    try {
        const { demarche, dept, cv, energie, annee } = req.body;
        const result = calculerCGServeur({ demarche, dept, cv: parseInt(cv), energie, annee: parseInt(annee) });
        res.json({ success: true, result });
    } catch (err) {
        console.error(err);
        res.status(400).json({ success: false, error: "Erreur lors du calcul: " + err.message });
    }
});
